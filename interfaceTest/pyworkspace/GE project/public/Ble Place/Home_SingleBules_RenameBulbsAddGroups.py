...
def Home_SingleBules_RenameBulbsAddGroups(self):
    u"""重命名多个灯，添加到多个分组中"""
    Time(1)
    try:
    ...
    if (self.driver.find_element('id', 'com.ge.cbyge:id/act_guide_pager_tv').text == 'Looking for bulbs...'):
        ...
        for j in range(0, num):
            ...
            self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/tv_login').text,
                             'Skip naming your bulbs',
                             '灯重命名保存失败')
            self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/view_bulb_edit_tv').text, rename,
                             '灯重命名保存失败')
            self.driver.find_element('id', 'com.ge.cbyge:id/view_bulb_edit_add').click()
            # list是列表名称，botton是勾选按钮,进入分组页面获取分组数信息
