...
class ContactsAndroidTests(ContactsAndroid):
    def Home_SingleBules_BuleInformation(self):
        u"""进入SingleBules列表单灯详情页面"""
        Time(1)
        try:
            self.assertIsNone(self.driver.find_element('name', 'Add a bulb').text, '没有添加灯按钮')
            sleep(3)
        except:
            self.find_by_scroll('Add a bulb')
        ...
        if (k > 0):
            ...
            sleep(3)
            self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/seekbar_light_lum_text').text, 'BRIGHTNESS',
                             '单灯详情查看失败')
        elif (k <= 0):
            print u"不存在灯"
...
#     suite.addTest(ContactsAndroidTests("Home_SingleBules_BuleInformation"))
