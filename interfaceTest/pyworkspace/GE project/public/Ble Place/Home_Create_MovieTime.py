...
class ContactsAndroidTests(ContactsAndroid):
    u"""创建MovieTime"""
    def Home_Create_MovieTime(self):
        Time(1)
        self.swipLeft(1000,0.90,0.15,0.50)
        sleep(3)
        # 不添加所以信息，提示信息检查
        self.driver.find_element('name', 'MOVIE TIME').click()
        sleep(3)
        ...
        self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/dialog_tips').text,
                         'Please select at least one light you want in this scene!', '不添加设备提示信息错误')
        ...
        self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/fg_new_scene_bulb_select_tips2').text,
                         'ON for MOVIE TIME', 'Setup1不选择灯操作失败')
        # etup1选择1个灯，点击next
        self.driver.find_element('id', 'com.ge.cbyge:id/id_2').click()
        sleep(3)
        # t为Setup1中设备数量
        ...
        if (t >= 1):
            ...
            sleep(3)
            self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/fg_new_scene_bulb_select_tips2').text,
                             'ON for MOVIE TIME', 'Setup1选择单灯灯操作失败')
            # Setup2中不添加设备点击下一步
            self.driver.find_element('name', 'Next').click()
            sleep(5)
            self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/id_1').text, 'Setup (3 of 3)',
                             'Setup2不添加设备操作失败')
            # t1为Setup2中设备数量
            # 返回Setup2
            self.driver.find_element('id', 'com.ge.cbyge:id/id_2').click()
