...
class ContactsAndroidTests(ContactsAndroid):
    u"""Home_Wakeup创建"""
    def Home_Create_Wakeup(self):
        Time(1)
        #Scene Setup界面cancel功能
        self.driver.find_element('name', 'WAKE UP').click()
        sleep(3)
        self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/id_1').text, 'Scene Setup', 'WakeUp打开失败')
        self.driver.find_element('name', 'Cancel').click()
        sleep(3)
        self.assertEqual(self.driver.find_elements('id', 'com.ge.cbyge:id/textview')[0].text, 'Home', 'WakeUp取消失败')
        sleep(3)
        #重新进入WAKE UP页面
        self.driver.find_element('name', 'WAKE UP').click()
        sleep(3)
        ...
        self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/id_1').text, 'Setup (1 of 2)', 'WakeUp创建页面步骤一显示错误')
        #不选择灯，直接点击Next按钮
        self.driver.find_element('name', 'Next').click()
        sleep(3)
        self.assertEqual(self.driver.find_element('id', 'com.ge.cbyge:id/dialog_tips').text,'Please select at least one light you want in this scene!','警告信息不正确信息不正确')
        self.assertEqual(self.driver.find_element('id','com.ge.cbyge:id/dialog_btn').text,'Okay, got it','警告确认按钮名称错误')
        ...
        if(t>1):
            ...
            for i in range(0,t):
                ...
                sleep(3)
                self.assertEqual(self.driver.find_elements('id', 'com.ge.cbyge:id/textview')[0].text, 'Home', 'WakeUp创建失败')
        else:
            print u"没有在线的灯"
